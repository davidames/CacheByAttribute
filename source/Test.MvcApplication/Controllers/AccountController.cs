...
using CacheByAttribute.Test.MvcApplication.Models;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            if (hasPassword)
            {
                if (ModelState.IsValid)
                {
                    IdentityResult result = await UserManager.ChangePasswordAsync(User.Identity.GetUserId(), model.OldPassword, model.Bob);
                    if (result.Succeeded)
                    {
                        ApplicationUser user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
                        await SignInAsync(user, false);
            ...
            else
            {
                ...
                if (ModelState.IsValid)
                {
                    IdentityResult result = await UserManager.AddPasswordAsync(User.Identity.GetUserId(), model.Bob);
                    if (result.Succeeded)
